...
`mapper goto <id>` and clicking a room both walk you there, building the route the way the MUSHclient mapper does: runs of directions fold into `run 2s8w47s22w10sw`, and anything that isn't a direction breaks the run and goes as its own command. That is exactly where a custom exit belongs, and **a custom exit beats the plain direction** between the same two rooms. That matters more than it sounds: 1,468 of the cexits that came across from MUSHclient read `open south;;south`, so the direction on its own walks you into a shut door.

Every command goes out on its own line, `;` and all split before sending. That is what MUSHclient does too — its world file has `enable_command_stack="y"` with `command_stack_character=";"`, so it splits client-side and Aardwolf never sees a stacked line. Hand the MUD `west;say huzzah` whole and it answers "unknown command". `mapper clickwalk off` if you'd rather not click to walk.
