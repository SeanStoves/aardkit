...
| 76 | [Search Window](Search%20Window/README.md) | 'search all' / 'eqsearch all' in a panel of its own, paged and fresh every run. Aardwolf has no tag for this the way it does for helpfiles, so it reads the report's own opening line and stops at the prompt — the Note block at the end is advice the MUD can reword, the prompt is structural. |
